<?php
namespace Opencart\Admin\Model\Extension\CesOcmod\Setting;
...
class Modification extends \Opencart\System\Engine\Model {
	...
	public function editModification($modification_id, $data) {
		$this->db->query("UPDATE `" . DB_PREFIX . "modification` SET `name` = '" . $this->db->escape($data['name']) . "', `code` = '" . $this->db->escape($data['code']) . "', `author` = '" . $this->db->escape($data['author']) . "', `version` = '" . $this->db->escape($data['version']) . "', `link` = '" . $this->db->escape($data['link']) . "', `xml` = '" . $this->db->escape($data['xml']) . "', `date_modified` = NOW() WHERE modification_id = '" . $modification_id . "'");
	}
	...
	public function getOtherExtensions($code) {
		$query = $this->db->query("SELECT `code` FROM `" . DB_PREFIX . "extension` WHERE `extension` != '" . $this->db->escape($code) . "'");

		return array_column($query->rows, 'code');
	}
}